...
func (s *SmartContract) sendData(APIstub shim.ChaincodeStubInterface, args []string) sc.Response {
	...
	var records__receiver Records
	json.Unmarshal(recordsAsBytes_receiver, &records__receiver)
	records__receiver = append(records__receiver, record)
	recordsAsBytes_receiver, err = json.Marshal(records__receiver)
	if err != nil {
		return shim.Error(err.Error())
	}
	//record to fabric
